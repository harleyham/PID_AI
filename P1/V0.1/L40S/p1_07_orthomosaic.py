...
def detect_projected_epsg(crs):
    """
    Tenta extrair o EPSG 'real' do CRS de forma robusta.
    Não depende só de to_epsg(), que pode retornar o geográfico base.
    """
    if crs is None:
        return None, None
...
def make_utm_to_enu_transformers(meta):
    """
    Cria os transformadores e parâmetros necessários para converter
    pontos UTM (EPSG do projeto) para ENU local do modelo COLMAP.
    """
    ...
    crs_geodetic = CRS.from_epsg(4979)   # lon, lat, h
...
def utm_xyz_to_enu(xyz_utm, tfm):
    """
    Converte pontos (N,3) em UTM para ENU local.
    xyz_utm[:,0] = Easting
    xyz_utm[:,1] = Northing
    xyz_utm[:,2] = altitude
    """
...
def camera_optical_axis_world(R):
    # eixo +Z da câmera em coordenadas do mundo
...
def bilinear_sample_rgb(src, u, v):
    """
    u, v em coordenadas de pixel (col, row).
    """
    if src.count < 3:
...
def maybe_make_preview(output_tif: Path, output_vrt: Path | None, output_preview: Path | None):
    ...
    if output_preview:
        try:
            subprocess.run(
                ["gdal_translate", "-of", "JPEG", "-outsize", "20%", "20%",
                 str(output_tif), str(output_preview)],
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True
...
def main():
    parser = argparse.ArgumentParser(description="Pipeline P1 - M07 Ortomosaico")
    parser.add_argument("--surface", required=True, help="DSM.tif ou DTM.tif")
    ...
    with rasterio.open(surface_path) as surf:
        ...
        metric(
            ...
            "surface_epsg",
            detected_epsg if detected_epsg is not None else "None",
            "code"
        )
        metric(
            metrics_csv, dataset, gpu, module,
            "surface_crs_string",
            crs_str if crs_str is not None else "None",
            "srs"
        )
        ...
        # força o CRS do ortho final para o EPSG do projeto
        try:
        ...
        # pré-calcular metadados das imagens/câmeras
        image_models = []
        for img in images:
            ...
            for row0 in range(0, out_height, tile_size):
                for col0 in range(0, out_width, tile_size):
                    ...
                    if not np.any(valid_flat):
                        ...
                        dst.write(tile_rgb[:, :, 0], 1, window=Window(col0, row0, tile_w, tile_h))
                        dst.write(tile_rgb[:, :, 1], 2, window=Window(col0, row0, tile_w, tile_h))
                        dst.write(tile_rgb[:, :, 2], 3, window=Window(col0, row0, tile_w, tile_h))
                        continue
                    ...
                    for cand in candidates:
                        ...
                        num_inside = int(np.count_nonzero(inside_img))
                        log_line(
                            log_file, dataset, gpu, module, "INFO",
                            f"Imagem {cand['name']} -> pixels projetados dentro da imagem: {num_inside}"
                        )
                        ...
                        if blend_mode == "first":
                            new_mask = np.all(best_rgb[flat_use_idx] == 0, axis=1)
                            if np.any(new_mask):
                    ...
                    tile_rgb = out_flat.reshape(tile_h, tile_w, 3)
                    ...
                    log_line(
                        log_file, dataset, gpu, module, "INFO",
                        f"Tile processado row={row0}:{row0+tile_h} col={col0}:{col0+tile_w}"
                    )
    ...
    metric(metrics_csv, dataset, gpu, module, "ortho_size_bytes", output_tif.stat().st_size if output_tif.exists() else 0, "bytes")
