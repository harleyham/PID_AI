...
def main():
    ...
    python_ok, python_path = check_executable(PYTHON_BIN)
    if python_ok:
        log_info(args.log_file, args.dataset, args.gpu, args.module,
                 f"OK binary: PYTHON_BIN ({python_path})")
        metric(args.metrics_csv, args.dataset, args.gpu, args.module,
               "binary_python", 1, "bool")
    else:
        msg = f"FALTA binary: PYTHON_BIN ({PYTHON_BIN})"
        log_error(args.log_file, args.dataset, args.gpu, args.module, msg)
        metric(args.metrics_csv, args.dataset, args.gpu, args.module,
               "binary_python", 0, "bool", "FAILED")
        failures += 1
