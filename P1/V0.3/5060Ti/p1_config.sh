...
# ------------------------------------------------------------
export PROJECT_ROOT="/media/ham1/EXT4/PROJETO_LIGEM_HIBRIDO"
...
# full     -> executa M00..M08
# from_m06 -> executa apenas M06 e M08, usando LAS externo
export PIPELINE_RUN_MODE="${PIPELINE_RUN_MODE:-full}"

# Origem lógica da nuvem
export DENSE_SOURCE_NAME="ODM" # ou "ODM"

# Arquivo LAS externo
export EXTERNAL_DENSE_LAS="${EXTERNAL_DENSE_LAS:-$PROJECT_ROOT/06_External_Processing/DS03/ODM_Densa_DS3.laz}"

# ------------------------------------------------------------
# Módulo 04 - limites da stereo_fusion
# ------------------------------------------------------------
export StereoFusion_num_threads="-1"  # 24 na L40S, -1 para outros

# Limites para reduzir pico de RAM no stereo_fusion
export StereoFusion_cache_size="16"
...
# Produtos finais
# Estrutura:
# 04_Produtos_Finais/<PIPELINE_PRODUCTS_DIR>/<DATASET_SLUG>/<GPU>
# ------------------------------------------------------------
# Nome da pasta-raiz dos produtos desse pipeline.
# Pode ser igual ao PIPELINE_NAME, ou diferente se você quiser
# separar "nome lógico" do pipeline e "nome de publicação".
export PIPELINE_PRODUCTS_DIR="${PIPELINE_PRODUCTS_DIR:-$PIPELINE_NAME}"

export OUTPUT_PRODUCTS_ROOT="$PROJECT_ROOT/04_Produtos_Finais/$PIPELINE_PRODUCTS_DIR"
export OUTPUT_DATASET_DIR="$OUTPUT_PRODUCTS_ROOT/$DATASET_SLUG"
...
if [[ "$PIPELINE_RUN_MODE" == "from_m06" ]]; then
    export DENSE_LAS="$EXTERNAL_DENSE_LAS"
else
    export DENSE_LAS="$OUTPUT_PATH/dense_utm_color.las"
fi

# Override opcional para nuvens externas sem CRS embutido.
# Deixe vazio para o M06 inferir o EPSG do LAS/LAZ.
export DENSE_LAS_EPSG="${DENSE_LAS_EPSG:-}"
...
OUTPUT_PRODUCTS_ROOT             : $OUTPUT_PRODUCTS_ROOT
