...
: "${PYTHON_BIN:=python3}"
: "${LAS_OUTPUT:=${DENSE_LAS:-$OUTPUT_PATH/dense_utm_color.las}}"
...
if [[ -n "${ENU_META_JSON:-}" && -f "$ENU_META_JSON" && -s "$ENU_META_JSON" ]]; then
    p1_metric "$MODULE" "enu_meta_exists" "1" "bool" "SUCCESS" "$ENU_META_JSON"
else
    p1_log_warn "$MODULE" "ENU_META_JSON ausente; EPSG será inferido do LAS/LAZ de entrada ou de DENSE_LAS_EPSG"
    p1_metric "$MODULE" "enu_meta_exists" "0" "bool" "WARNING" "M06_infer_epsg_from_las"
fi
...
p1_metric "$MODULE" "dense_las_epsg_override" "${DENSE_LAS_EPSG:-}" "code"
