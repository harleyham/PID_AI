...
def epsg_from_crs_json(value):
    ...
    if not isinstance(value, dict):
        return None
    crs_id = value.get("id")
    if isinstance(crs_id, dict) and str(crs_id.get("authority", "")).upper() == "EPSG":
        try:
            return int(crs_id["code"])
        except Exception:
            return None
    return None


def epsg_from_wkt(value):
    if not value:
        return None
    matches = re.findall(r'AUTHORITY\s*\[\s*["\']EPSG["\']\s*,\s*["\']?(\d+)["\']?\s*\]', str(value))
    if not matches:
        return None
    # Em WKT projetado, o ultimo AUTHORITY costuma ser o EPSG do CRS horizontal.
    return int(matches[-1])


def epsg_from_proj4(value):
    if not value:
        return None
    text = str(value)
    ...
    return 32700 + zone if "+south" in text else 32600 + zone


def epsg_from_srs_dict(srs):
    if not isinstance(srs, dict):
        return None

    epsg = epsg_from_crs_json(srs.get("json"))
    if epsg:
        return epsg

    for key in ("compoundwkt", "horizontal", "wkt", "prettycompoundwkt", "prettywkt"):
        epsg = epsg_from_wkt(srs.get(key))
        if epsg:
            return epsg

    return epsg_from_proj4(srs.get("proj4"))
...
def epsg_from_pdal_summary(summary):
    for item in iter_dicts(summary):
        epsg = epsg_from_srs_dict(item.get("srs"))
        if epsg:
            return epsg, "las_srs"
        epsg = epsg_from_crs_json(item.get("json"))
        if epsg:
            return epsg, "las_srs_json"
        for key in ("spatialreference", "comp_spatialreference"):
            epsg = epsg_from_wkt(item.get(key))
            if epsg:
                return epsg, f"las_{key}"
    return None, None


def epsg_from_las_header(path):
    try:
...
def resolve_epsg(args, summary, log_file, dataset, gpu, module):
    if args.epsg:
        try:
            return int(args.epsg), "DENSE_LAS_EPSG"
        except ValueError:
            raise RuntimeError(f"EPSG informado invalido: {args.epsg}")

    epsg, source = epsg_from_pdal_summary(summary)
    if epsg:
        return epsg, source

    epsg = epsg_from_las_header(args.dense_las)
    if epsg:
        return epsg, "las_header"

    if args.enu_meta_json:
        enu_meta_json = Path(args.enu_meta_json)
        if enu_meta_json.exists() and enu_meta_json.stat().st_size > 0:
            meta = read_json(enu_meta_json)
            if "epsg" in meta:
                return int(meta["epsg"]), "ENU meta"
            log_info(log_file, dataset, gpu, module, f"ENU meta sem campo epsg: {enu_meta_json}")

    raise RuntimeError(
        "Nao foi possivel determinar EPSG. Informe DENSE_LAS_EPSG no p1_config.sh "
        "ou use um LAS/LAZ com CRS embutido."
    )
...
def main():
    ...
    parser.add_argument("--enu-meta-json", default="")
    parser.add_argument("--epsg", default=os.environ.get("DENSE_LAS_EPSG", ""))
    ...
    enu_meta_json = Path(args.enu_meta_json) if args.enu_meta_json else None
    ...
    try:
        epsg, epsg_source = resolve_epsg(args, summary, log_file, dataset, gpu, module)
    except Exception as exc:
        log_error(log_file, dataset, gpu, module, str(exc))
        return 2
